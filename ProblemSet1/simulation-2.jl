...
include("MSEK.jl")
include("counterfactual.jl") # Run the WIOD code to get the trade shares.

#Set up the exact data I want to use to parameterise the model.

# simulation
J = 56
N = 44
t = rand(Uniform(.01,.2),J,N,N) #Currently random
Π = [I + rand(Pareto(1,.001),N,N) for j=1:J] #Currently random
Π = [Π[j] ./ sum(Π[j],dims=1) for j=1:J] 
Π = cat(addDim.(Π,1)...,dims=1) 
Y = rand(LogNormal(0,2.),N) #Currently random
D = rand(Uniform(-.02,.02),N) .* Y #Currently random
D = D .- mean(D)
γ = [ I + rand(Pareto(1,.01),J,J) for n=1:N ] #Currently random
γ = cat([ rand(Uniform(.2,.3),1,J) .* γ[n] ./ sum(γ[n],dims=1) for n=1:N ]...,dims=3) #Currently random
μ = rand(Uniform(0,1),J,N) #Currently random
μ = μ ./ sum(μ,dims=1)
θ = rand(Uniform(2,8),J) #Currently random
