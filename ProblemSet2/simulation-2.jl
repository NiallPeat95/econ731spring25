...
Π_l = rand(Pareto(1, 0.001), J, N)
Π_l = Π_l./ sum(Π_l, dims=1)

# Generate other random values
Y = rand(LogNormal(0, 2.),J, N)
D = vec(rand(Uniform(-0.02, 0.02), N) .* sum(Y,dims=1)')  # Trade deficits as a fraction of total output
D = D .- mean(D)
